...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            # results_folder = base_results_path  # The base folder where the result is stored add gotv
            results_path = '{}.txt'.format(base_results_path)
            results_file = '{}_001.txt'.format(seq.name)  # The specific result file
            # results_path = os.path.join(results_folder, results_file)  # Join folder and file name add gotV
